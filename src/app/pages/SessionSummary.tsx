...
export function SessionSummary() {
  ...
  const { song, correct, total } = location.state || { song: '', correct: 0, total: 0 };
  
  const percentage = Math.round((correct / total) * 100);
  
  const getMessage = () => {
    if (percentage === 100) {
      return '¡Excelente trabajo! Conoces muy bien esta canción';
    } else if (percentage >= 80) {
      return '¡Muy bien! Lo estás haciendo de maravilla';
    } else if (percentage >= 60) {
      return '¡Buen trabajo! Cada práctica te ayuda a mejorar';
    } else {
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-28 h-28 bg-wine/10 rounded-3xl mb-6">
            <Award className="w-16 h-16 text-wine" strokeWidth={2} />
          </div>
          
          <h2 className="text-[40px] mb-4 text-wine">
            ¡Sesión completada!
          </h2>
        ...
        <div className="bg-card rounded-2xl p-8 shadow-xl mb-8">
          <div className="flex items-center gap-4 mb-6 pb-6 border-b border-warm-beige-dark">
            <Music2 className="w-8 h-8 text-wine flex-shrink-0" strokeWidth={2} />
            <div>
              <p className="text-[18px] text-warm-gray mb-1">Canción practicada</p>
              <p className="text-[24px] text-warm-black">{song}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="bg-olive-green/10 rounded-xl p-6 text-center">
              ...
              <p className="text-[18px] text-olive-green-dark mb-2">Aciertos</p>
            ...
            <div className="bg-wine/10 rounded-xl p-6 text-center">
              ...
              <p className="text-[18px] text-wine-dark mb-2">Progreso</p>
          ...
          {/* Tiempo aproximado */}
          <div className="mt-6 pt-6 border-t border-warm-beige-dark flex items-center justify-center gap-3 text-warm-gray">
            <Clock className="w-6 h-6" strokeWidth={2} />
            <p className="text-[18px]">Tiempo aproximado: {Math.ceil(total * 0.5)} minutos</p>
          </div>
        </div>
        ...
        {percentage < 100 && (
          <div className="bg-deep-blue/10 rounded-xl p-6 mb-8 text-center">
            <p className="text-[20px] text-deep-blue-dark leading-relaxed">
              La práctica hace al maestro. Cada vez que practicas, fortaleces tu memoria y disfrutas más de la música.
            </p>
          </div>
        )}
